...
import com.twosixlabs.dart.auth.tenant.{CorpusTenant, GlobalCorpus}
...
trait ReduxProviderProps extends js.Object {
    ...
    var loader : React.Element = js.native
...
trait JsDartContextProviderDI {
    ...
    object JsDartContextProvider {
        ...
        private def component( context : JsDartContextProviderProps )( eles : VdomElement* ) =
            cmp( context )( eles : _* )
        ...
        case class Props(
            ...
            def buildJsContext : JsDartContext = {
                ...
                ctx.tenants = backend.user.toSet.flatMap( ( usr : DartUser ) => usr.groups.map {
                    case ProgramManager => "global"
                    case TenantGroup( tenant, _ ) => tenant match {
        ...
        val ContextBuilder : ReactComponent[ Props, Unit ] = ReactComponent.functional[ Props ] {
            case props@Props( _, _, _, _, _, child ) =>
