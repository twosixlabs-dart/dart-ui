...
function NumberComponentPopup({
  ...
  const [componentData, setComponentData] = React.useState({
    type: componentTypes.METADATA_NUMBER_COMPONENT,
    label: 'Word Count',
    layout: {
      x: 0,
      y: 0,
    ...
    aggs: {
      [aggId]: getAggQuery('word_count', 100),
    },
    state: {
      field: 'word_count',
      bucketSize: 100,
  ...
  const supportedFields = Object.values(fields).filter((f) => f.data_type === 'int')
...
NumberComponentPopup.propTypes = {
...
export default connect(mapStateToProps)(withStyles(styles)(NumberComponentPopup));
