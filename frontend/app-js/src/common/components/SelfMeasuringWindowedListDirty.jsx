...
class SelfMeasuringWindowedListDirty extends Component {
  constructor(props) {
    super(props);
  ...
  // componentDidMount() {
  //   window.addEventListener('resize', this.initializeRowHeights);
  // }
  ...
  componentDidUpdate() {
    ...
    if (this.displayedRows) {
      ...
      if (updatedRows.length > 0) {
        ...
        if (updatedRows[0] === 0) rowHeightsArray[0].totalSize = rowHeightsArray[0].height;
        else {
  ...
  //   window.removeEventListener('resize', this.initializeRowHeights);
  // }
  ...
  getWindowDimensions() {
    if (this.rowHeightsArray[0].totalSize !== undefined) {
      ...
      while (windowStartIndex < rowCount - 1
        && this.rowHeightsArray[windowStartIndex].totalSize < scrollOffset) windowStartIndex += 1;
      const startIndex = windowStartIndex - overscanCount < 0
        ? 0 : windowStartIndex - overscanCount;
      let paddingBefore = 0;
      if (startIndex > 0) paddingBefore = this.rowHeightsArray[startIndex - 1].totalSize;
  ...
  measureAll() {
    if (this.rowToMeasure === null || this.rowToMeasure === undefined) {
      return;
    }
    ...
    const rowStart = this.rowToMeasure;
    const rowLimitExc = rowStart + (2 * overscanCount);
    const rowLimit = rowLimitExc > rowCount ? rowCount : rowLimitExc;

    window.requestIdleCallback(() => {
      if (this.rowToMeasure !== rowStart) {
        return;
      }
      ...
      const allElements = rowHeightsArray
        .slice(rowStart, rowLimit)
        .map((v, i) => {
          const row = i + rowStart;
          if (rowHeightsArray[row].measured) return <div />;
          return (
            <div
      ...
      const elementsWithContext = measureContextInjector(allElements);

      const handleRenderedElements = () => {
        let min = null;
        for (let i = rowStart; i < rowLimit; i += 1) {
          if (this.rowToMeasure !== rowStart) {
            return;
          }
          if (!rowHeightsArray[i].measured) {
            const iEle = document.getElementById(`row-for-measurement-${i}`);
            let newHeight = 0;
            if (iEle !== null && iEle !== undefined) {
              newHeight = iEle.clientHeight;
            }
            rowHeightsArray[i].height = newHeight;
            rowHeightsArray[i].measured = true;
            if (min === null) min = i;
      ...
      ReactDOM.render(elementsWithContext, invisibleContainer, handleRenderedElements);
    });
  }
...
SelfMeasuringWindowedListDirty.propTypes = {
  measureContextInjector: PropTypes.func,
...
SelfMeasuringWindowedListDirty.defaultProps = {
  measureContextInjector: (ele) => ele,
...
export default withStyles(styles)(SelfMeasuringWindowedListDirty);
